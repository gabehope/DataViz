...
class InteractiveCell extends BaseCell {
    ...
    async runCode(code) {
        ...
        try {
            ...
            // Process result
            let output = await mainPyodide.runPythonAsync(code);

            if(typeof output.to_json === 'function') {
                graphFigure.appendChild(Inputs.table(JSON.parse(output.to_json(null, 'records'))));
                output = "";
            }
            else if(typeof output.to_json === 'function') {
                await vegaEmbed(graphFigure, JSON.parse(output.to_json()));
                output = "";
            }
