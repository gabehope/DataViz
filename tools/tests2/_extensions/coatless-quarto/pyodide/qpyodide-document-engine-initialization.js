...
globalThis.qpyodideInstance = await import(
  qpyodideCustomizedPyodideOptions.indexURL + "pyodide.mjs").then(
   async({ loadPyodide }) => {
    ...
    // Unregister the `js` package to prevent javascript execution
    mainPyodide.unregisterJsModule('js');
    await mainPyodide.runPythonAsync('import sys;del sys.modules["js"]');
