...
globalThis.cellContainer = new CellContainer();
...
class BaseCell {
    ...
    async executeCode() {
        ...
        const result = await this.runCode(this.code);
        return result;
    }

    async autoRunExecuteCode() {
        // Execute code using Pyodide
...
class InteractiveCell extends BaseCell {
    ...
    constructor(cellData) {
        ...
        globalThis.cellContainer.addCell(this);
    }
    ...
    setupElement() {
        ...
        runCodeButton.className = 'btn btn-light btn-xs qpyodide-button qpyodide-button-run';
        runCodeButton.type = 'button';
        runCodeButton.id = `qpyodide-button-run-${this.id}`;
        runCodeButton.disabled = true;
        runCodeButton.innerHTML = '<i class="fas fa-circle-notch fa-spin"></i>';
        ...
        clearButton.className = 'btn btn-light btn-xs qpyodide-button qpyodide-button-clear';
        clearButton.type = 'button';
        clearButton.id = `qpyodide-button-clear-${this.id}`;
        clearButton.title = 'Clear output';
        clearButton.innerHTML = '<i class="fa-regular fa-circle-xmark"></i>';


        // Append buttons to the rightButtonsDiv
        rightButtonsDiv.appendChild(clearButton);
        ...
        if (this.options['type'] != 'output') {
            consoleAreaDiv.appendChild(toolbarDiv);
            consoleAreaDiv.appendChild(editorDiv);
        }
        
        consoleAreaDiv.appendChild(outputCodeAreaDiv);

        if ((this.options['output-location'] === 'right') || (this.options['output-location'] === 'left') ) {
            consoleAreaDiv.classList.add("qpyodide-columns");
            outputGraphAreaDiv.classList.add("qpyodide-columns");
            consoleAreaDiv.classList.add("column");
            outputGraphAreaDiv.classList.add("column");
        } 
        if ((this.options['output-location'] === 'left') || (this.options['output-location'] === 'top')) {
        ...
        toolbarDiv.classList.add("hide-toolbar");
        mainDiv.onmouseenter = function() {
            toolbarDiv.classList.remove("hide-toolbar");
        }
    ...
    setupMonacoEditor() {
        ...
        this.clearButton = document.getElementById(`qpyodide-button-clear-${this.id}`);
        ...
        require(['vs/editor/editor.main'], function () {
            thiz.editor = monaco.editor.create(
                thiz.editorDiv, {
                    ...
                    lineNumbers: thiz.options['line-numbers'] ?? false
                }
            );
        ...
        thiz.clearButton.onclick = function () {
            thiz.clearOutput();
        };
    ...
    disableInteractiveCells() {
        ...
        this.runButton.innerHTML = '<i class="fas fa-circle-notch fa-spin"></i>';
    }
    ...
    enableInteractiveCells() {
        ...
        this.runButton.innerHTML = '<i class="fas fa-play"></i>';
    }

    clearOutput() {
        // Clear the output area of content
    ...
    async runCode(code) {
        ...
        graphFigure.className = `qpyodide-interactive-area`;
        ...
        try {
            ...
            let {results, error} = await mainPyodide(code, {});
            if (results) {
                output = results;
            } else {
                output = error;
            ...
            if(results.type === 'dataframe') {
                graphFigure.appendChild(Inputs.table(results.data));
                output = "";
            }
            else if(results.type === 'altair') {
                await vegaEmbed(graphFigure, JSON.parse(results.data));
                output = "";
            }
        } catch (e) {
            // Add error message
